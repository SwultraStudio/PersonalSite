...
const Posts = () => {
    ...
    useEffect(() => {
        async function getPosts() {
            try {
                let response = await fetch("https://wkzacnpyvqeytgqzbzsv.supabase.co/rest/v1/posts?select=*", {
                    method: 'GET',
                    headers: {
    ...
    return (
        ...
        <Helmet>
            ...
            <meta name="description" content="مقالات تتعلق بتطوير البرمجيات احمد محمد" />
        </Helmet>

        <div dir='rtl' className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8  min-h-screen">
            <h2 className="text-3xl font-bold mb-4 pt-3">المقالات</h2>
            {IsLoading && <FontAwesomeIcon icon={faSpinner} className="h-8 w-8 text-blue-600 animate-spin" aria-hidden="true"/>}
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                {postsInfo ? (
                    postsInfo.map(post => (
                        <a href={"/Post?Id=" + post.id}  className="block bg-white rounded-lg shadow-md overflow-hidden hover:shadow-xl transition duration-300">
